#!/usr/bin/env bash
# Continue with loaded support, swing clearance and AMP 0.25. Stop the old trainer first.
set -euo pipefail
...
training_console="logs/stairs_safe_landing_$(date +%Y%m%d_%H%M%S).log"
...
bash scripts/run_with_rsl5.sh scripts/rsl_rl/train.py \
 ...
 --run_name safe_landing_v9 --resume --load_run "$(dirname "$resume_checkpoint")" \
