#!/usr/bin/env bash
# Continue with virtual edge volumes and toe/riser guidance. Stop the old trainer first.
set -euo pipefail
...
training_console="logs/stairs_instinct_edges_$(date +%Y%m%d_%H%M%S).log"
...
bash scripts/run_with_rsl5.sh scripts/rsl_rl/train.py \
 ...
 --run_name instinct_edges_toe_v8 --resume --load_run "$(dirname "$resume_checkpoint")" \
