#!/usr/bin/env bash
# Continue with turning practice, heading slowdown and guarded rough-terrain progression. Stop the old trainer first.
set -euo pipefail
...
training_console="logs/stairs_turning_rough_$(date +%Y%m%d_%H%M%S).log"
...
bash scripts/run_with_rsl5.sh scripts/rsl_rl/train.py \
 ...
 --run_name turning_rough_v10 --resume --load_run "$(dirname "$resume_checkpoint")" \
